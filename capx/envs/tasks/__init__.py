...
def _register_openarm() -> None:
    from .openarm.openarm_motion import OpenArmMotionCodeEnv

    register_exec_env("openarm_motion_code_env", OpenArmMotionCodeEnv)
    register_config(
        "openarm_motion_code_env",
        CodeExecEnvConfig(
            low_level="openarm_real_low_level",
            apis=["OpenArmControlApi"],
        ),
    )


def _register_franka_core() -> None:
    ...
    register_exec_env("two_arm_lift_code_env", TwoArmLiftCodeEnv)
    register_config(
        "two_arm_lift_code_env",
        CodeExecEnvConfig(
            low_level="two_arm_lift_robosuite",
            apis=["FrankaTwoArmLiftApi"],
        ),
    )


def _register_franka_libero() -> None:
...
_safe_register("OpenArm task envs", _register_openarm)
_safe_register("Franka task envs", _register_franka_core)
_safe_register("Franka LIBERO task envs", _register_franka_libero)
_safe_register("R1Pro task envs", _register_r1pro)
